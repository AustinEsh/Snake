...
def best_snake_weights(population_name, generation, pop_size):
    ...
    path = os.path.join(sourceFileDir, "weights/stats.txt")
    f = open(path, "r")
    lines = f.readlines()
    f.close()
    last = lines[-1]
    ...
    weights_path = os.path.join(sourceFileDir, "weights/generation_" + str(generation) + ".txt")
...
def get_stats_as_history(population_name):
    sourceFileDir = os.path.dirname(os.path.abspath(__file__))
    #path = "weights/genetic_algorithm/" + str(population_name) + "/stats.txt"
    path = os.path.join(sourceFileDir, "weights/genetic_algorithm/" + str(population_name) + "/stats.txt")
...
if __name__ == "__main__":
    ...
    if restore_weights_from_txt:
        ...
        sourceFileDir = os.path.dirname(os.path.abspath(__file__))
        path = os.path.join(sourceFileDir, "weights/generation_" + str(current_gen) + ".txt")
    ...
    for generation in range(num_generations):
        ...
        sourceFileDir = os.path.dirname(os.path.abspath(__file__))
        path = os.path.join(sourceFileDir, "weights/genetic_algorithm/" + str(population_name) + "/")
        ...
        if generation % checkpoint == 0 or generation == num_generations-1:
            ...
            sourceFileDir = os.path.dirname(os.path.abspath(__file__))
            path = os.path.join(sourceFileDir, "weights/genetic_algorithm/" + str(population_name) + "/generation_" + str(generation) + ".txt")
