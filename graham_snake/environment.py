...
class Environment():
    ...
    def __init__(self, waitTime, boardSize = 8, numApples = 5):
        
        # Defining the parameters
        self.width = 880            # width of the game window
        self.height = 880           # height of the game window
        self.nRows = boardSize             # number of rows in our board
        self.nColumns = boardSize          # number of columns in our board
    ...
    def moveSnake(self, nextPos, col):
        ...
        if col:
            ...
            self.applePos.pop(self.applePos.index(nextPos)) # self.snakePos))
            self.applePos.append(self.placeApple())
            self.collected = True
            
        for i in range(len(self.applePos)):
            self.screenMap[self.applePos[i][0]][self.applePos[i][1]] = 1
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_SPACE and not start:
                    start = True
                if (event.key == pg.K_i or event.key == pg.K_UP or event.key == pg.K_e) and direction != 1:
                    direction = 0
                elif (event.key == pg.K_l or event.key == pg.K_RIGHT or event.key == pg.K_f) and direction != 3:
                    direction = 2
                elif (event.key == pg.K_j or event.key == pg.K_LEFT or event.key == pg.K_s) and direction != 2:
                    direction = 3
                elif (event.key == pg.K_k or event.key == pg.K_DOWN or event.key == pg.K_d) and direction != 0:
                    direction = 1
